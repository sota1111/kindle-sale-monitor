...
Kindleで購入したい漫画を登録しておき、**Amazon Product Advertising API (PA-API 5.0)** を主データ源として（PA-API 認証情報が無い場合はセール情報サイト「sale-bon」のスクレイピングにフォールバック）価格・セールを監視し、Discordに通知するシステムです。

> **構成（SOT-1299）**: デプロイされる Web は **表示専用 + データ登録（入力の Firestore 登録）のみ** です。
> リサーチ（価格・セール監視）は Web からは実行せず、**ローカルから CLI `python scripts/run_research.py`** で実行します。
> これに伴い、本番（Cloud Run）の定期監視（Cloud Scheduler / アプリ内スケジューラ / 手動チェックボタン）は停止しています。
...
- リサーチ（価格・セール監視）はローカル CLI `python scripts/run_research.py` で実行（cron 等で定期化を想定）
...
Web（Cloud Run）とリサーチ（ローカル）を分離し、Firestore を共有データストアとします。

```
[ローカル] python scripts/run_research.py
  ├──► データ取得（data_source ディスパッチャ）
  │       ├─ Amazon PA-API 5.0 GetItems（主データ源）
  │       └─ sale-bon.com スクレイピング（フォールバック）
  ├──► SQLite → Firestore ミラー（セール状態・履歴・通知履歴）
  └──► Discord Webhook（新規セール検出時のみ）

[Cloud Run] Web (FastAPI) — 表示専用 + 入力の Firestore 登録のみ
  ├──► Firestore から復元（rehydrate）してデータ表示
  └──► 管理画面からの登録（本リスト・通知条件・設定）を Firestore へ保存
```

> リサーチ系のコード（`app/services/checker.py`, `app/services/scheduler.py`,
> `app/api/check.py`, `app/api/run.py`, `app/api/scheduler.py`）はリポジトリに残していますが、
> Web にはマウントされていません（CLI から利用します）。
...
# 6. 動作確認
curl http://localhost:8000/healthz        # ヘルスチェック（Web は表示専用）

# 7. リサーチ（監視処理）はローカル CLI から実行する
python scripts/run_research.py            # 価格・セール監視を実行（結果は Firestore ミラー経由で保存）
```
...
**注意（SOT-1299）**: Web からはリサーチ用エンドポイント（`POST /run` / `POST /api/check` / `/api/scheduler/*`）は
マウントされなくなりました。監視はローカル CLI `python scripts/run_research.py` で実行してください。
...
### 3. Cloud Schedulerの設定（SOT-1299 以降は不要・非推奨）

> **非推奨（SOT-1299）**: Web からリサーチ用 `POST /run` を撤去したため、Cloud Scheduler による
> 本番定期監視は使用しません。リサーチはローカルで `python scripts/run_research.py` を cron などにより
> 定期実行してください。以下は旧構成の参考情報です。
